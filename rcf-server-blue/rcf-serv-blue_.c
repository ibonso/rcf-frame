...
int main(int argc, char *argv[])
{
    ...
    openlog(SYSLOGNAME, LOG_CONS | LOG_PID, LOG_DAEMON); //LOG_PERROR

    syslog(LOG_INFO, "rcf-serv-b start. Socket:%s", socket_path);
    ...
    for ( ; ; ) {
        ...
        if( rc<0 ) {
           syslog(LOG_ERR, "rcf-serv-b child socket:%s",socket_path);
